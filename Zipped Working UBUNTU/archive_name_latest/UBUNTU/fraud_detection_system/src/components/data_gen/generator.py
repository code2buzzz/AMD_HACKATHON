...
def data_exists() -> bool:
    ...
    if not os.path.exists(SYNTHETIC_DATA_DIR):
        return False
    return all(
        os.path.exists(os.path.join(SYNTHETIC_DATA_DIR, f)) for f in REQUIRED_FILES
    )
...
def generate_data():
    ...
    print(
        "🚀 Starting synthetic fraud data generation (Sequential IDs & Unique Transactions)..."
    )
    ...
    # ----------------------------------------------------
    sanction_data = []
    # MODIFIED: Sequential zero-padded IDs
    sanction_ids = [f"SANC_{i+1:05d}" for i in range(COUNTS["sanction_list"])]
    ...
    customer_data = []
    # MODIFIED: Sequential zero-padded IDs
    customer_ids = [f"CUST_{i+1:05d}" for i in range(COUNTS["customers"])]
    ...
    device_data = []
    # MODIFIED: Sequential zero-padded IDs
    device_ids = [f"DEV_{i+1:05d}" for i in range(COUNTS["devices"])]
    ...
    beneficiary_data = []
    # MODIFIED: Sequential zero-padded IDs
    beneficiary_ids = [f"BENE_{i+1:05d}" for i in range(COUNTS["beneficiaries"])]
    ...
    merchant_data = []
    # MODIFIED: Sequential zero-padded IDs
    merchant_ids = [f"MERCH_{i+1:05d}" for i in range(COUNTS["merchants"])]
    ...
    # MODIFIED: Used enumerate to inject sequential relational mapping IDs
    cust_devices_data = [
        {
            "customer_device_id": f"CDEV_{idx+1:05d}",
            "customer_id": p[0],
            "device_id": p[1],
            "first_seen": datetime.now() - timedelta(days=random.randint(30, 300)),
            "last_seen": datetime.now() - timedelta(days=random.randint(1, 29)),
        }
        for idx, p in enumerate(cust_dev_pairs)
    ]
    ...
    cust_benes_data = [
        {
            "customer_beneficiary_id": f"CBEN_{idx+1:05d}",
            "customer_id": p[0],
            "beneficiary_id": p[1],
        ...
        for idx, p in enumerate(cust_bene_pairs)
    ]
    df_customer_beneficiaries = pd.DataFrame(cust_benes_data)

    # ----------------------------------------------------
    # 7. TRANSACTIONS (STANDARDIZED CASE CONSTANTS)
    # ----------------------------------------------------
    transaction_data = []
    # Enforce standard uppercase character layouts across both scripts
    tx_ids = [f"TX_{uuid.uuid4().hex.upper()}" for _ in range(COUNTS["transactions"])]

    cust_to_devs = df_customer_devices.groupby("customer_id")["device_id"].apply(list).to_dict()
    cust_to_benes = df_customer_beneficiaries.groupby("customer_id")["beneficiary_id"].apply(list).to_dict()
    ...
    for i in range(COUNTS["transactions"]):
        ...
        d_id = random.choice(cust_to_devs[c_id]) if c_id in cust_to_devs else random.choice(device_ids)
        is_p2p = random.random() > 0.4
        
        b_id = (random.choice(cust_to_benes[c_id]) if c_id in cust_to_benes else random.choice(beneficiary_ids)) if is_p2p else None
        m_id = random.choice(merchant_ids) if not is_p2p else None

        is_fraud_infrastructure = (d_id == mule_device) or (b_id == mule_bene)

        # Standarized enum strings matching live generation constants
        if is_p2p:
        ...
        amount = round(random.uniform(5000, 50000), 2) if is_fraud_infrastructure else round(random.uniform(5, 1200), 2)

        orig_ctry = "IN"
        dest_ctry = orig_ctry if not is_fraud_infrastructure else random.choice(["KY", "RU", "KP"])
        is_intl = orig_ctry != dest_ctry

        status = "DECLINED" if (is_fraud_infrastructure and random.random() > 0.5) else random.choice(["APPROVED", "APPROVED", "PENDING"])
        ...
        transaction_data.append({
            ...
            "merchant_id": m_id,
            "device_id": d_id,
            "transaction_timestamp": (start_timeline + timedelta(minutes=int(i * (180 * 24 * 60 / COUNTS["transactions"])))).strftime("%Y-%m-%dT%H:%M:%SZ"),
    ...
    # ----------------------------------------------------
    # 8. CUSTOMER BEHAVIOR LOGS (STANDARDIZED)
    # ----------------------------------------------------
    behavior_data = []
    behavior_ids = [f"BEH_{i+1:05d}" for i in range(COUNTS["customer_behavior"])]
    tx_sample_pool = df_transactions.sample(n=COUNTS["customer_behavior"], replace=False).to_dict(orient="records")
    ...
    for i in range(COUNTS["customer_behavior"]):
        ...
        tx_time = datetime.strptime(tx_ref["transaction_timestamp"], "%Y-%m-%dT%H:%M:%SZ")
        login_t = (tx_time - timedelta(minutes=random.randint(2, 15))).strftime("%Y-%m-%d %H:%M:%S")
        logout_t = (tx_time + timedelta(minutes=random.randint(1, 10))).strftime("%Y-%m-%d %H:%M:%S")
        ...
        behavior_data.append({
            ...
            "session_duration_minutes": random.randint(5, 25) if is_bad else random.randint(1, 4),
            "behavior_risk_score": round(random.uniform(75.0, 100.0), 2) if is_bad else round(random.uniform(0.0, 40.0), 2),
            "account_takeover_suspected": is_bad and random.random() > 0.3,
            "transaction_frequency_24h": random.randint(12, 45) if is_bad else random.randint(1, 4),
            "avg_transaction_amount_7d": round(random.uniform(8000, 60000), 2) if is_bad else round(random.uniform(10, 500), 2),
            "failed_transaction_count_24h": random.randint(3, 10) if is_bad else 0,
            ...
            "fraud_flag": is_bad,
            "fraud_risk": "CRITICAL" if is_bad else "LOW",
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        })
    df_customer_behavior = pd.DataFrame(behavior_data)

    # ----------------------------------------------------
    # 9. TRANSACTION ANALYSIS LOGS (STANDARDIZED)
    # ----------------------------------------------------
    analysis_data = []
    analysis_ids = [f"ANAL_{i+1:05d}" for i in range(COUNTS["transaction_analysis_logs"])]
    ...
    for i in range(COUNTS["transaction_analysis_logs"]):
        ...
        is_graph_fraud = (tx_ref["device_id"] == mule_device or tx_ref["beneficiary_id"] == mule_bene)

        b_score = behavior_map[tx_id]["behavior_risk_score"] if has_behavior else round(random.uniform(0, 30), 2)
        g_score = round(random.uniform(85, 100), 2) if is_graph_fraud else round(random.uniform(0, 35), 2)
        s_score = round(random.uniform(90, 100), 2) if tx_ref["destination_country"] in ["KY", "RU", "KP"] else round(random.uniform(0, 10), 2)

        overall = max(b_score, g_score, s_score)
        prob = round(overall / 100.0, 2)

        category = "CRITICAL" if overall > 75 else "MEDIUM" if overall > 40 else "LOW"
        decision = "TERMINATE_TRANSACTION" if overall > 75 else "ALLOW_TRANSACTION"

        agent_1 = {"agent_name": "VelocityParser", "status": "flagged" if b_score > 60 else "clear"}
        agent_2 = {"agent_name": "DeviceGraphSpy", "status": "shared_mule_pool" if is_graph_fraud else "isolated"}
        agent_3 = {"agent_name": "SanctionScanner", "matches": 1 if s_score > 50 else 0}
        agent_4 = {"agent_name": "Geolocator", "distance_miles": random.randint(2000, 7000) if is_graph_fraud else random.randint(0, 15)}
        agent_5 = {"agent_name": "LLMSummarizer", "verdict": "Suspicious pattern tracking cluster" if is_graph_fraud else "Safe footprint"}
        ...
        analysis_data.append({
            ...
            "recommended_action": "Freeze Account immediately" if overall > 75 else "None",
            "investigation_status": "Open" if overall > 75 else "Closed",
            "report": b"Raw investigation binary dump logs context text metrics",
            "created_at": tx_ref["created_at"]
        })
