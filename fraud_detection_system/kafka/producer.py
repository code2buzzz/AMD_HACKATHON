...
# MODIFIED: Imported COUNTS to enforce absolute data alignment boundaries
from config.settings import KAFKA_CONFIG, COUNTS
...
# MODIFIED: Refactored arrays to use zero-padded sequential IDs matching the ingestor data definitions
CUSTOMERS = [f"CUST_{i+1:05d}" for i in range(COUNTS["customers"])]
FRAUD_RING_CUSTOMERS = CUSTOMERS[
    :45
]  # Matches the first 45 accounts tracked as seed mules

DEVICES = [f"DEV_{i+1:05d}" for i in range(COUNTS["devices"])]
HIGH_RISK_DEVICES = DEVICES[:10]  # First 10 hardware instances act as flagged assets

# MODIFIED: Standardized prefix to MERCH_ to match table definitions inside graph_ingestion.py
MERCHANTS = [f"MERCH_{i+1:05d}" for i in range(COUNTS["merchants"])]
HIGH_RISK_MERCHANTS = MERCHANTS[:3]

# MODIFIED: Standardized prefix to BENE_ to match lookup constraints in Cypher templates
...
print(f"[*] Initialized {len(CUSTOMERS)} Customers and {len(DEVICES)} Devices.")
print("[*] Unified ID schema definitions. Pumping live events to Kafka...")
...
try:
    while True:
        ...
        # MODIFIED: Enforced unique TX_ prefix + full-hex uuid payload to ensure zero collision insert rates
        ...
        if fraud_profile == "FRAUD_RING":
            ...
            session_duration_minutes = random.randint(12, 45)
            device_risk_score = round(random.uniform(20.0, 60.0), 2)
            typing_speed_flag = False
            biometric_anomaly_detected = random.random() < 0.65
